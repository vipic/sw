define("./workbox-7502dbf5.js", ["exports"], (function (t) {
  "use strict";
  try {
    ...
    addCacheListener() {
      self.addEventListener("message", t => {
        if (t.data && "CACHE_URLS" === t.data.type) {
          const {payload: e} = t.data, s = Promise.all(e.urlsToCache.map(t => {
            "string" == typeof t && (t = [t]);
  ...
  class y {
    ...
    async open() {
      if (!this.o) return this.o = await new Promise((t, e) => {
        let s = !1;
        setTimeout(() => {
  ...
  for (const [t, e] of Object.entries(g)) for (const s of e) s in IDBObjectStore.prototype && (y.prototype[s] = async function (e, ...n) {
  ...
  class R {
    constructor(t) {
      this.m = t, this.o = new y("workbox-expiration", 1, {onupgradeneeded: t => this.R(t)})
    }
    ...
    async setTimestamp(t, e) {
      const s = {url: t = m(t), timestamp: e, cacheName: this.m, id: this.q(t)};
    ...
    async expireEntries(t, e) {
      const s = await this.o.transaction("cache-entries", "readwrite", (s, n) => {
        const i = s.objectStore("cache-entries").index("timestamp").openCursor(null, "prev"), r = [];
        let a = 0;
        i.onsuccess = () => {
          const s = i.result;
          if (s) {
  ...
  class q {
    constructor(t, e = {}) {
      this.v = !1, this.U = !1, this.L = e.maxEntries, this.N = e.maxAgeSeconds, this.m = t, this._ = new R(t)
    }

    async expireEntries() {
      if (this.v) return void (this.U = !0);
      this.v = !0;
      const t = this.N ? Date.now() - 1e3 * this.N : 0, e = await this._.expireEntries(t, this.L),
  ...
  const v = (t, e) => t.filter(t => e in t), U = async ({request: t, mode: e, plugins: s = []}) => {
    ...
    const r = await self.caches.open(t), a = await U({plugins: i, request: e, mode: "read"});
    let c = await r.match(a, n);
    for (const e of i) if ("cachedResponseWillBeUsed" in e) {
    ...
    const h = await (async ({request: t, response: e, event: s, plugins: n = []}) => {
      let i = e, r = !1;
      for (const e of n) if ("cacheWillUpdate" in e) {
        r = !0;
    ...
    if (!h) return;
    const u = await self.caches.open(t), l = v(r, "cacheDidUpdate"),
    ...
    for (const e of l) await e.cacheDidUpdate.call(e, {
      cacheName: t,
      event: i,
      oldResponse: f,
      newResponse: h,
  ...
  }, N = L, b = async ({request: t, fetchOptions: e, event: n, plugins: i = []}) => {
    if ("string" == typeof t && (t = new Request(t)), n instanceof FetchEvent && n.preloadResponse) {
      const t = await n.preloadResponse;
      if (t) return t
    }
    const r = v(i, "fetchDidFail"), a = r.length > 0 ? t.clone() : null;
    try {
      for (const e of i) if ("requestWillFetch" in e) {
        const s = e.requestWillFetch, i = t.clone();
    ...
    try {
      ...
      for (const t of i) "fetchDidSucceed" in t && (s = await t.fetchDidSucceed.call(t, {
        event: n,
        request: c,
        response: s
      }));
      ...
      for (const e of r) await e.fetchDidFail.call(e, {
        error: t,
        event: n,
  ...
  async function K(t, e) {
    const s = t.clone(), n = {headers: new Headers(s.headers), status: s.status, statusText: s.statusText},
      i = e ? e(n) : n, r = function () {
        if (void 0 === E) {
  ...
  class D {
    constructor(t) {
      this.m = f(t), this.K = new Map, this.M = new Map, this.D = new Map
    }
    ...
    addToCacheList(t) {
      ...
      for (const n of t) {
        ...
        const {cacheKey: t, url: i} = M(n), r = "string" != typeof n && n.revision ? "reload" : "default";
        if (this.K.has(i) && this.K.get(i) !== t) throw new s("add-to-cache-list-conflicting-entries", {
          firstEntry: this.K.get(i),
          secondEntry: t
        });
        if ("string" != typeof n && n.integrity) {
          if (this.D.has(t) && this.D.get(t) !== n.integrity) throw new s("add-to-cache-list-conflicting-integrities", {url: i});
          this.D.set(t, n.integrity)
        }
    ...
    async install({event: t, plugins: e} = {}) {
      const s = [], n = [], i = await self.caches.open(this.m), r = await i.keys(), a = new Set(r.map(t => t.url));
      for (const [t, e] of this.K) a.has(e) ? n.push(t) : s.push({cacheKey: e, url: t});
      const c = s.map(({cacheKey: s, url: n}) => {
        const i = this.D.get(s), r = this.M.get(n);
        return this.O({cacheKey: s, cacheMode: r, event: t, integrity: i, plugins: e, url: n})
      });
      await Promise.all(c);
    ...
    async activate() {
      const t = await self.caches.open(this.m), e = await t.keys(), s = new Set(this.K.values()), n = [];
      for (const i of e) s.has(i.url) || (await t.delete(i), n.push(i.url));
      return {deletedURLs: n}
    }

    async O({cacheKey: t, url: e, cacheMode: n, event: i, plugins: r, integrity: a}) {
      const c = new Request(e, {integrity: a, cache: n, credentials: "same-origin"});
      let o, h = await b({event: i, plugins: r, request: c});
      for (const t of r || []) "cacheWillUpdate" in t && (o = t);
      if (!(o ? await o.cacheWillUpdate({
        event: i,
        request: c,
        response: h
      }) : h.status < 400)) throw new s("bad-precaching-response", {url: e, status: h.status});
      h.redirected && (h = await K(h)), await x({
        event: i,
        plugins: r,
        response: h,
    ...
    createHandlerBoundToURL(t, e = !0) {
      ...
      const n = this.createHandler(e), i = new Request(t);
      return () => n({request: i})
    }
  }
  ...
  function k(t) {
    C || ((({ignoreURLParametersMatching: t = [/^utm_/], directoryIndex: e = "index.html", cleanURLs: s = !0, urlManipulation: n} = {}) => {
      ...
      self.addEventListener("fetch", r => {
        ...
        if (!a) return;
        let c = self.caches.open(i).then(t => t.match(a)).then(t => t || fetch(a));
        r.respondWith(c)
      })
    })(t), C = !0)
  }
  ...
  const A = [], S = {
    ...
    const e = T(), s = S.get();
    t.waitUntil(e.install({event: t, plugins: s}).catch(t => {
      throw t
    }))
  }, F = t => {
    const e = T();
  ...
  t.CacheFirst = class {
    constructor(t = {}) {
      this.m = w(t.cacheName), this.T = t.plugins || [], this.P = t.fetchOptions, this.C = t.matchOptions
    }

    async handle({event: t, request: e}) {
      "string" == typeof e && (e = new Request(e));
      let n, i = await N({cacheName: this.m, request: e, event: t, matchOptions: this.C, plugins: this.T});
      if (!i) try {
        i = await this.k(e, t)
      } catch (t) {
        n = t
    ...
    async k(t, e) {
      const s = await b({request: t, event: e, fetchOptions: this.P, plugins: this.T}), n = s.clone(),
    ...
    constructor(t = {}) {
      ...
      }, this.B = t, this.N = t.maxAgeSeconds, this.F = new Map, t.purgeOnQuotaError && (e = () => this.deleteCacheAndMetadata(), p.add(e))
    }

    S(t) {
      if (t === w()) throw new s("expire-custom-caches-only");
      let e = this.F.get(t);
      return e || (e = new q(t, this.B), this.F.set(t, e)), e
    }
    ...
    !function (t) {
      T().addToCacheList(t), t.length > 0 && (self.addEventListener("install", B), self.addEventListener("activate", F))
    }(t), k(e)
...
//# sourceMappingURL=workbox-7502dbf5.js.map
